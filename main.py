...
*{{{{COUPON INFO / DISCOUNT %}}}}* (IF AVAILABLE, AND ONLY IF OLD_PRICE IS GREATER THAN CURRENT_PRICE)
...
6. When there is no discount, OMIT only the discount line
...
def load_api_keys() -> Dict[str, str]:
    ...
    print("Loaded API:", api_keys_raw[:25] + "...")
...
def initialize_processing_context(staging_output_dir: str) -> dict:
    ...
    context = {
        ...
        "needs_captcha_refresh": False,  # Flag to signal CAPTCHA refresh needed before next URL
        "captcha_encountered_flag": False,  # Flag to signal CAPTCHA was encountered during current scrape
    ...
    try:
        ...
        if os.path.exists(json_path):
            with open(json_path, "r", encoding="utf-8") as f:
                try:
                    data = json.load(f)
                    print(f"[DEBUG] Loaded {len(data)} existing products")
                except json.JSONDecodeError as e:
                    print("=" * 60)
                    print("JSON IS CORRUPTED!")
                    print(e)
                    print(f"File size: {os.path.getsize(json_path)}")
                    print("=" * 60)
                    return False
        ...
        print(f"[DEBUG] Saving {len(data)} total products")
        print(f"[DEBUG] Writing to: {os.path.abspath(json_path)}")
        print(f"[DEBUG] Final count before write: {len(data)}")
...
def process_urls_pipeline(args, urls_to_process, total_urls, api_keys, context):
    """
    Orchestrates the scraping pipeline using the BrowserPool for concurrent processing.

    Creates a pool of 3 reusable browser instances and distributes URLs across them
    using a thread-safe queue. Browsers that encounter CAPTCHAs are automatically
    blocked and their URLs requeued for another available browser.
    ...
    print(f"Remaining URLs: {len(urls_to_process)}")

    for i, u in enumerate(urls_to_process[:10]):
        print(f"{i}: {repr(u)}")
    ...
    # Initialize and start the browser pool with 3 reusable Chrome instances
    pool = BrowserPool(
        num_browsers=3,
        cooldown=180,
        api_keys=api_keys,
        save_callback=lambda product_data, url: save_product_data_json(
            product_data,
    ...
    # Extract plain URLs from tuples
    plain_urls = [item[0] if isinstance(item, tuple) else item for item in urls_to_process]

    # Submit all URLs to the pool
    for url in plain_urls:
        pool.submit_url(url)

    # Monitor pool progress
    total = len(plain_urls)
    print(f"\n{BackgroundColors.BOLD}{BackgroundColors.GREEN}Processing {total} URLs with 3 concurrent browsers...{Style.RESET_ALL}")

    try:
        # Wait for queue to drain with periodic status updates
        while True:
            queue_size = pool.url_queue.qsize()
            status = pool.get_status()
            available = sum(1 for b in status["browsers"].values() if b["status"] == "AVAILABLE")
            blocked = sum(1 for b in status["browsers"].values() if b["status"] == "BLOCKED")
            processed = status["total_processed"]
            print(f"\r{BackgroundColors.CYAN}[Pool] Processed: {processed}/{total} | "
                  f"Queued: {queue_size} | "
                  f"Available: {available} | "
                  f"Blocked: {blocked}{Style.RESET_ALL}", end="", flush=True)
            if processed >= total:
                break
            time.sleep(2)
    except KeyboardInterrupt:
        print(f"\n{BackgroundColors.YELLOW}Interrupted by user. Shutting down pool...{Style.RESET_ALL}")
    finally:
        print()  # newline after progress
        pool.shutdown(wait=True)

    final_status = pool.get_status()
    context["successful_scrapes"] = final_status["total_processed"]
    print(f"\n{BackgroundColors.GREEN}Pool processing complete. Processed: {final_status['total_processed']}/{total}{Style.RESET_ALL}")
